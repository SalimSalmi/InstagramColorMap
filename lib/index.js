...
const writer = csvWriter({headers: ["city", "Id", "Filter", "Created_Time", "Longitude", "Latitude", "Likes", "Comments", "Average_Color"]})

var dataset = require("../data/unparsed-data");
writer.pipe(fs.createWriteStream('data/parsed-data.csv'));
...
function loopDataset(index){
  for(let i = index; i < index+1000; i++){
    ...
    getColors(dataset[i].Images.Thumbnail.Url, function(err, colors){
      ...
      result.push(chroma.average(colors).hex());
